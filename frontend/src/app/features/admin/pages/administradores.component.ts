...
@Component({
  ...
  template: `
    <div class="content-page">
      ...
      <!-- â•â• Painel 1: FormulÃ¡rio de criaÃ§Ã£o / ediÃ§Ã£o â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• -->
          ...
          <div class="form-grid-2">
            ...
            <div class="field span2">
              ...
              @if (tocados.has('nome') && !adminForm.nome.trim()) {
                <span class="field-error">Campo obrigatÃ³rio</span>
              }
            </div>
            ...
            <div class="field">
              ...
              <div class="email-wrap">
                ...
                @if (emailStatus === 'available') { <span class="email-status badge badge-success">DisponÃ­vel</span> }
                @if (emailStatus === 'taken') { <span class="email-status badge badge-danger">JÃ¡ cadastrado</span> }
              </div>
            </div>
            ...
            <div class="field">
              ...
              @if (tocados.has('confirmarEmail') && adminForm.confirmarEmail.trim() && adminForm.confirmarEmail.toLowerCase() !== adminForm.email.toLowerCase()) {
                <span class="field-error">Os e-mails nÃ£o conferem</span>
              }
            </div>
            ...
            <div class="field">
              ...
              <div class="password-wrap">
                <input class="field-input"
                  ...
                  placeholder="MÃ­nimo 6 caracteres" (blur)="marcarTocado('senha')" />
              ...
              @if (tocados.has('senha') && !editandoAdminId && !adminForm.senha) {
                <span class="field-error">Senha obrigatÃ³ria</span>
              }
              @if (tocados.has('senha') && adminForm.senha && adminForm.senha.length < 6) {
                <span class="field-error">MÃ­nimo 6 caracteres</span>
              }
            </div>
            ...
            <div class="field">
              ...
              @if (tocados.has('confirmarSenha') && !editandoAdminId && adminForm.confirmarSenha && adminForm.confirmarSenha !== adminForm.senha) {
                <span class="field-error">As senhas nÃ£o conferem</span>
              }
            </div>
            ...
            <div class="convites-section">
              ...
              @if (carregandoConvites) {
                ...
                <div class="table-wrap">
                  <table class="table">
                    <thead><tr><th>E-mail</th><th>Perfil</th><th>Status</th><th>Expira em</th><th>AÃ§Ã£o</th></tr></thead>
                    <tbody>
      ...
      <div class="card card-elevated">
            ...
            <table class="table table-zebra">
              <thead>
                <tr>
                  ...
                  <th>Status</th>
                  <th>Cursos</th>
                  @if (p.can('administradores:write')) { <th>AÃ§Ãµes</th> }
                </tr>
              </thead>
              <tbody>
                        ...
                        <div class="cursos-panel card-flat">
                          ...
                          <div class="cursos-checkboxes">
                            ...
                            @if (cursosDisponiveis.length === 0) {
                              <p style="color:var(--color-text-muted);font-size:var(--font-size-sm)">Nenhum curso disponÃ­vel.</p>
                            }
                          </div>
        ...
        @if (!carregandoLista && totalPaginas > 1) {
          <div class="pagination">
            <button class="btn btn-ghost btn-sm" (click)="irParaPagina(1)" [disabled]="paginaAtual === 1">Â«</button>
            <button class="btn btn-ghost btn-sm" (click)="irParaPagina(paginaAtual - 1)" [disabled]="paginaAtual === 1">â€¹</button>
            ...
            <button class="btn btn-ghost btn-sm" (click)="irParaPagina(paginaAtual + 1)" [disabled]="paginaAtual === totalPaginas">â€º</button>
            <button class="btn btn-ghost btn-sm" (click)="irParaPagina(totalPaginas)" [disabled]="paginaAtual === totalPaginas">Â»</button>
            <span style="color:var(--color-text-muted);font-size:var(--font-size-sm)">PÃ¡gina {{ paginaAtual }} de {{ totalPaginas }}</span>
          </div>
        }
      </div>
    </div>
...
export class AdminAdministradoresComponent implements OnInit, OnDestroy {
  ...
  get forcaSenha(): { score: number; texto: string; cor: string } {
    ...
    const textos = ['', 'Fraca', 'RazoÃ¡vel', 'Boa', 'Forte'];
  ...
  private readonly ROLE_DESCRIPTIONS: Record<AdminRole, string> = {
    ...
    instrutor:   'Gerencia cursos, aulas, provas, notas e presenÃ§a. Recebe acesso automÃ¡tico apenas aos cursos que criar. Acesso adicional pode ser concedido manualmente.',
  };
  ...
  formatCursosAdmin(cursoIds: number[]): string {
    ...
    // Se todos os cursos disponÃ­veis estÃ£o selecionados
    if (
    ...
    // Trunca para caber no layout: acumula nomes atÃ© 60 caracteres
    const MAX_CHARS = 60;
    let resultado = '';
  ...
  // â”€â”€ Convites â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  onFotoSelecionada(event: Event): void {
    ...
    if (!['image/jpeg', 'image/png'].includes(file.type)) {
      this.adminFormErro = 'Apenas JPG e PNG sÃ£o aceitos.';
      input.value = '';
      return;
    }
    ...
    if (file.size > 5 * 1024 * 1024) {
      this.adminFormErro = 'A imagem deve ter no mÃ¡ximo 5MB.';
      input.value = '';
      return;
    }
    ...
    // Upload para o servidor â€” salva o arquivo em disco
    ...
    this.http.post<any>(
      ...
      next: (resp) => {
        ...
        // Armazena apenas a URL do arquivo em disco (nÃ£o base64)
  ...
  excluirAdmin(admin: AdminResumo): void {
    if (!confirm(`Excluir o administrador "${admin.nome}" permanentemente? Esta aÃ§Ã£o nÃ£o pode ser desfeita.`)) return;
  ...
  salvarAdmin() {
    ...
    if (
      ...
      this.adminFormErro = 'Nome e e-mail sÃ£o obrigatÃ³rios.';
      return;
    }
    ...
    if (
      ...
      this.adminFormErro = 'Os e-mails nÃ£o conferem.';
      return;
    }

    if (this.emailStatus === 'taken') {
      this.adminFormErro = 'Este e-mail jÃ¡ estÃ¡ cadastrado. Escolha outro.';
      return;
    }

    // ValidaÃ§Ãµes apenas para criaÃ§Ã£o (novo admin)
    if (!this.editandoAdminId) {
      if (!this.adminForm.senha.trim() || !this.adminForm.confirmarSenha.trim()) {
        this.adminFormErro = 'Senha Ã© obrigatÃ³ria para novo administrador.';
        return;
      }

      if (this.adminForm.senha !== this.adminForm.confirmarSenha) {
        this.adminFormErro = 'As senhas nÃ£o conferem.';
        return;
      }
    ...
    if (this.editandoAdminId) {
      // EdiÃ§Ã£o: enviar apenas os campos alterÃ¡veis
      const payload = {
      ...
      // CriaÃ§Ã£o: enviar todos os campos incluindo senha
      const payload = {
        nome: this.adminForm.nome,
        email: this.adminForm.email,
  ...
  // â”€â”€ MÃ¡scaras â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
